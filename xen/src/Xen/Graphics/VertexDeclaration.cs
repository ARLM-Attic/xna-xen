...
namespace Xen.Graphics
{
	...
	class VertexDeclarationBuilder
	{
		...
		class ElementHash : IComparable<ElementHash>
		{
			...
			public int CompareTo(ElementHash othervh)
			{
				...
				for (int i = 0; i < elements.Length; i++)
				{
					...
					if (elements[i].Stream != other[i].Stream)
						return (int)elements[i].Stream - (int)other[i].Stream;
					...
					if (elements[i].VertexElementMethod != other[i].VertexElementMethod)
						return (int)elements[i].VertexElementMethod - (int)other[i].VertexElementMethod;
		...
		public VertexDeclaration GetDeclaration(GraphicsDevice device, Type[] streamTypes, IVertices[] buffers)
		{
			...
			lock (hashingDecl)
			{
				hashingDecl.SetFrom(streamTypes,typeHash,ref typeIndex);
				if (declarationHash.TryGetValue(hashingDecl, out declaration))
					return declaration;
			}
				...
				foreach (VertexElement el in map)
				{
					...
					foreach (VertexElement check in mapping)
					{
						if (el.UsageIndex == check.UsageIndex &&
							el.VertexElementUsage == check.VertexElementUsage)
						{
							skip = true;
							break;
						}
			...
			if (declaration == null)
				declaration = new VertexDeclaration(device, elements);
			declarationHash.Add(new DeclarationHash(streamTypes, typeHash, ref typeIndex), declaration);

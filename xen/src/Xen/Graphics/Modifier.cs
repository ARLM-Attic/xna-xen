...
namespace Xen.Graphics.Modifier
{
	...
	/// <summary>
	/// A simple <see cref="IBeginEndDraw"/> drawable that modifies the <see cref="Xen.Graphics.State.DepthColourCullState.ColourWriteMask"/>, the previous mask is reset when rendering is complete.
	/// </summary>
#if !DEBUG_API
	...
	public sealed class ColourMaskModifier : IBeginEndDraw
	{
		...
		{
			if (currentlyEnabled)
				state.RenderState.DepthColourCull.ColourWriteMask = previous;
	...
	{
		private float left, right, top, bottom;
		private bool enabled = true, currentlyActive, currentlyEnabled, previousEnabled;
		...
		void IBeginEndDraw.Begin(DrawState state)
		{
			...
			if (enabled)
			{
				...
				previousEnabled = device.RenderState.ScissorTestEnable;
		...
		void IBeginEndDraw.End(DrawState state)
		{
			if (currentlyEnabled)
			{
				...
				device.RenderState.ScissorTestEnable = previousEnabled;
			}
			currentlyActive = false;
			currentlyEnabled = false;
		}

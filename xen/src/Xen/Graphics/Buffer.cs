...
namespace Xen.Graphics
{
		...
		where DataType : struct
	{
		...
		protected abstract void WriteBlock(DrawState state, DataType[] data, int sourceStartIndex, int writeOffsetBytes, int copyElements, object target, object parent);
		protected abstract void WriteComplete();

		public int WriteBuffer(DrawState state, int startIndex, int bytesToWrite, object target, object parent)
		{
			IEnumerable<DataType> array = this.data;
			...
			if (array is DataType[])
			{
				...
				//	if (size <= maxsize)
				{
					WriteBlock(state, data, startIndex, startIndex * instanceStride, bytesToWrite / instanceStride, target, parent);
					WriteComplete();
				}
			...
			if (count != -1)
			{
				...
				try
				{
					...
					if (startIndex == 0 && writeBuffer.Length == count && array is IList<DataType>)
				...
				if (writeBuffer != null)
				{
					WriteBlock(state, writeBuffer, 0, startIndex * instanceStride, bytesToWrite / instanceStride, target, parent);
					WriteComplete();
					return bytesToWrite / instanceStride;
				}
			}
			...
			lock (writeBuffer)
			{
				...
				foreach (DataType item in array)
				{
					...
					{
						WriteBlock(state, writeBuffer, 0, startIndex * instanceStride, inc, target, parent);

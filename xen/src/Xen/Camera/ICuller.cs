...
namespace Xen
{
	...
	internal static class FrustumCull
	{
		...
		internal static bool BoxInFrustum(Plane[] frustum, ref Vector3 minExtents, ref Vector3 maxExtents, ref Matrix world, float baseRadius)
		{
			if (baseRadius != 0)
			{
				//as nasty as this looks this is very fast in release, much faster than the bounding box test.
				//the expensive bounding box test need only be done when a sphere test _intersects_ with the frustum (not when entirely inside or outside)
				//this radius should always be lager than the box, no matter what.
				//sometimes it will be significantly larger, but that isn't the point :-)
				//	float radius = Math.Max(Math.Max(Math.Max(world.M11, world.M12), Math.Max(world.M21, world.M22)),
				...
				foreach (Plane plane in frustum)
				{
					float d = plane.Normal.X * world.M41 + plane.Normal.Y * world.M42 + plane.Normal.Z * world.M43 + plane.D;
					if (d > baseRadius)
						return false;
					if (d > -baseRadius)
						overlap = true;

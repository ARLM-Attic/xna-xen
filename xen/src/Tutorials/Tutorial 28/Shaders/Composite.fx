...
float4 OutputColour(float3 colour)
{
	...
	{
		//Note: the 0.75 here is to compensate for this method producing slighty brighter output than the other two
		//Normally this isn't needed.
		colour.rgb *= 0.75;
		
		//run the approximation
		float3 rgb = max(0, colour.rgb - 0.004);
		colour.rgb = rgb * (0.5 + 6.2 * rgb) / (0.06 + rgb * (1.7 + 6.2 * rgb));

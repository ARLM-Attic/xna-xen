				...
				out float3 normalOut		: TEXCOORD0)
{
	...
	// Using .xyz on position prevents the position from being translated (only rotated)
	// As output depth value is Z / W, using .xyww should always generate depth of 1.0 (max depth)
	
	positionOut = mul(position.xyz,worldViewProj).xyww;		// * float4(1,1,1,1.000001); // nvidia cards don't like this trick!

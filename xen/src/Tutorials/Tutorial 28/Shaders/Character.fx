				...
				out float4 shadowMapSample	: TEXCOORD5)
{
	...
	tangentOut		= mul(tangent.xyz,world);
	binormOut		= mul(binorm.xyz,world);
...
//blending vertex shader
void CharacterBlendVS(	
					float4 position			: POSITION, 
				out float4 positionOut		: POSITION,
					float2 texCoordIn		: TEXCOORD0,
				out float2 texCoordOut		: TEXCOORD0,
					...
					int4	indices			: BLENDINDICES)
{
	...
	//vertex position after blend transform   
	...
	//rotate normals/tangents by blend matrix
	normal			= mul(normal.xyz,(float3x3)blendMatrix);
	tangent			= mul(tangent.xyz,(float3x3)blendMatrix);
	binorm			= mul(binorm.xyz,(float3x3)blendMatrix);
...
technique CharacterBlend
{
   pass
   {
		VertexShader = compile vs_3_0 CharacterBlendVS();
		PixelShader = compile ps_3_0 CharacterPS();
   }
}
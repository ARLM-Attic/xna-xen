...
namespace Tutorials.Tutorial_19
{
	...
	//Also, after the model animations have been generates, it allows the bones to be
	...
	//they should be thread safe with any draw code (async animations are usually processed during a Draw).
	//
	//
	//Important note about animations in Xen.
	//
	//One aspect of Xen animation may be difficult to grasp.
	//In xen, if all bones are identity transforms (ie, no rotation or translation), then the
	//model will be drawn in it's default pose. (eg, the Tiny model's pose has outstretched arms)
	//This was done partly for performance reasons, and simplifying certain effects, such as weighting.
	//
	//Because of this however, directly modifying a bone transform may have highly unexpected results.
	//For example, if the bone for Tiny's head was set to a transform that only rotates 180 degrees,
	//(ie, the transform has no translation) then the result will be unexpected.
	//In such a case, the head will *not* rotate around centre of the bone. The head will rotate around
	//the centre of the model origin (0,0,0) - in this case, the head will rotate around Tiny's feet.
	//Because Tiny's head is far from her feet, the head will potentially rotate in very a large circle.
	//
	//Because of this, a bone transform should rotate around the centre of the bone's world space position.
	//
	//To achieve this, each method in the IAnimationBoneModifier interface passes in two read only arrays.
	//
	//These arrays represent the world space transforms of each bone, and also the inverse world space
	//transforms of each bone.
	//To rotate around the centre of the bone, use the following logic:
	//
	// transform = inverseWorldBoneTransform * (transform * worldBoneTransform);
	//
	//What this means, is the world bone transform is rotated by the user transform. This produces
	//the desired rotation, but in terms of the world space bone tranform. So the final step is to
	//transform by the inverse of the world transform. This produces the required tranform to
	...
	public class Tutorial : Application
	{
		...
		{
			text.Font = manager.Load<SpriteFont>("Arial");
		}
	...
	class SimpleVerletSolver
	{
		 ...
		 * 
		 * In other words, Please don't use this class as a base for more complex effects :-)
		 * 
		 */
		...
		{
			SkeletonData skeleton = modelData.Skeleton;
		...
		{
			BoneData bone = skeleton.BoneData[boneIndex];
			Matrix matrix;
			Transform transform;

//CompilerOptions = NoPreShader, InternalClass, ParentNamespace
...
)
{
	...
	float2 axis = float2(-1,-1);
	
	tex0 = tex + sampleDirection * 4 * axis;
	tex1 = tex + sampleDirection * 3 * axis;
	tex2 = tex + sampleDirection * 2 * axis;
	tex3 = tex + sampleDirection * 1 * axis;
	tex4 = tex + sampleDirection * -0 * axis;
	tex5 = tex + sampleDirection * -1 * axis;
	tex6 = tex + sampleDirection * -2 * axis;
	tex7 = tex + sampleDirection * -3 * axis;
}
...
)
{
	...
	tex0 = tex + sampleDirection * 2 * axis;
	tex1 = tex + sampleDirection * 1 * axis;
	tex2 = tex + sampleDirection * -0 * axis;
	tex3 = tex + sampleDirection * -1 * axis;
}
...
)
{
	...
	tex0 = tex + sampleDirection * 1 * axis;
	tex1 = tex + sampleDirection * 0 * axis;
}
float4 PS_Down8(

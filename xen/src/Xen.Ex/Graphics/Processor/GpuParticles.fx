//CompilerOptions = InternalClass, ParentNamespace, NoPreShader, UseAsmToHlslXboxConverter

float2 invTargetSize;
float3 indices128[128];

void GetPosition(float3 data, out float4 position, out float2 lifeData)
{
	float index				= data.x;
	index					*= invTargetSize.x;
	
	float2 xy				= float2(fmod(index,1),(floor(index+1))*invTargetSize.y);
...
{
	GetPosition(indices128[position.x],positionOut,texCoordOut);
}

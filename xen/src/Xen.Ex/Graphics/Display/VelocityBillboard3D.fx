...
float3 viewPoint : VIEWPOINT : register(c8);
float2 velocityScale : register(c9);
float3 worldSpaceYAxis : register(c10);

//stores the inverse texture size (XY) and the starting particle offset (Z)
float3 invTextureSizeOffset	: register(c11);


#ifndef XBOX360 // no cpu shaders on the xbox

//for large numbers of CPU particles
float4 positionData[75] : register(c12);
float4 velocityData[75] : register(c87);
float4 colourData[75]	: register(c162);
				...
				uniform bool	useUserPositionOffset)
{
	...
	float3 worldPoint = mul(positionOut,worldMatrix);
...
float4 ParticlePS(float2 texcoord : TEXCOORD0, float4 colour : TEXCOORD1) : COLOR0
{
	...
	colour *= tex2D(DisplaySampler,texcoord.xy);

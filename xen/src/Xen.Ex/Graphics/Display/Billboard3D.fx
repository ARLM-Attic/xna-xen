...
float3 worldSpaceYAxis : register(c9);

//stores the inverse texture size (XY) and the starting particle offset (Z)
float3 invTextureSizeOffset	: register(c10);


#ifndef XBOX360 // no cpu shaders on the xbox

//for large numbers of CPU particles
float4 positionData[75] : register(c11);
float4 velocityData[75] : register(c86);
float4 colourData[75]	: register(c161);
				...
				uniform bool	useUserPositionOffset)
{
	...
	float3 worldPoint = mul(positionOut,worldMatrix) - viewPoint;

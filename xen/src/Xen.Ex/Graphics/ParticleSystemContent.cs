...
namespace Xen.Ex.Graphics.Content
{
	...
	internal interface IParticleProcessor : IDisposable
	{
		...
		void DrawProcess(DrawState state, int step);
	...
	public sealed class ParticleSystemRuntimeLogicData
	{
		...
		internal ParticleSystemRuntimeLogicData(ParticleSystemTypeData typeData, bool usesColour, CpuParticleProcessorData cpuData, TargetPlatform targetPlatform)
		{
			...
			this.gpuSystemData = new GpuParticleProcessorData(typeData, usesColour, usesUserValues, usesLifeOrAge, targetPlatform);
			this.cpuSystemData = cpuData;
		}

		internal void Write(BinaryWriter writer, TargetPlatform taretPlatform)
		{
			writer.Write(usesLifeOrAge);
			writer.Write(usesUserValues);

			gpuSystemData.Write(writer);
			if (taretPlatform != TargetPlatform.Xbox360)
	...
	public sealed class ParticleSystemData
	{
		...
		/// <param name="targetPlatform"></param>
		public void Write(BinaryWriter writer, TargetPlatform targetPlatform)
		{
	...
	public sealed class ParticleSystemTypeData
	{
		...
		internal void CreateRuntimeLogic(bool coloursUsed, CpuParticleProcessorData cpuData, TargetPlatform targetPlatform)
		{
			this.runtimeLogic = new ParticleSystemRuntimeLogicData(this, coloursUsed, cpuData, targetPlatform);
		}
		...
		internal void Write(BinaryWriter writer, TargetPlatform targetPlatform, bool includeLogicTypeData)
		{
			writer.Write(name);
		...
		internal ParticleSystemTypeData(ContentReader reader, int index, string basePath)
		{
			...
			this.blend = (AlphaBlendState)reader.ReadInt32();
		...
		internal void UpdateTextures(ContentManager content, string basePath)
		{
			...
			if (basePath == null) // hotloading?
			{
				...
				this.texture = Texture2D.FromFile(device, this.textureName, TextureCreationParameters.Default);
			}
			else
#endif

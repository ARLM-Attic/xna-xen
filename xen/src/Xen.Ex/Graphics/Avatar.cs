...
new Matrix(1f ,4.135903E-25f ,4.135903E-25f ,0f ,-4.135903E-25f ,1f ,0f ,0f ,-4.135903E-25f ,0f ,1f ,0f ,-0.03209561f ,-0.01738983f ,0.01951019f ,0.9999999f),
		...
		public AvatarInstance(Microsoft.Xna.Framework.GamerServices.AvatarDescription avatarDescription, bool useLoadingEffect)
		{
			...
			this.avatarRenderer = new Microsoft.Xna.Framework.GamerServices.AvatarRenderer(avatarDescription, useLoadingEffect);
			...
			set
			{
				if (value != avatarDescription)
				{
...
#endif
					{
						...
						this.avatarRenderer = new Microsoft.Xna.Framework.GamerServices.AvatarRenderer(value, this.useLoadingEffect);
					}
					else
		...
		/// <summary>
		/// Returns true if the avatar has been loaded and is ready for display
		/// </summary>
		public bool AvatarIsLoaded
		{
#if XBOX360
			get { return avatarRenderer.IsLoaded; }
#else
			get { return true; }
#endif
		}
		...
		public void Draw(DrawState state)
		{
			...
			if (lights != null && lights.LightingEnabled)
			{
				...
				bool isIdentity;
				Matrix world;
				state.GetWorldMatrix(out world, out isIdentity);
				if (!isIdentity)
				{
				...
				Vector3 directional = new Vector3();
				Vector3 lightDirection = new Vector3();

				//first must iterate all the lights, to get an approximate weighted direction for them
				AccumulateLightDirection(lights.plights, ref centre, ref lightDirection);
				AccumulateLightDirection(lights.vlights, ref centre, ref lightDirection);
				...
				AccumulateLightContribution(lights.plights, ref centre, ref lightDirection, ref ambient, ref directional, directionalWeight);
				AccumulateLightContribution(lights.vlights, ref centre, ref lightDirection, ref ambient, ref directional, directionalWeight);

				//finally, apply it to the model.
				avatarRenderer.AmbientLightColor = ambient;
				avatarRenderer.LightColor = directional;
			...
			bool isLoaded = avatarRenderer.IsLoaded;
			if (isLoaded || useLoadingEffect)
			{
				if (isLoaded && this.bindPoseIsDirty)
				{
					CalculateBindPose();
				}
		...
		private void AccumulateLightDirection(List<ShaderLight> lights, ref Vector3 centre, ref Vector3 lightDirection)
		{
			...
			Vector3 rgbScalar = new Vector3(0.3f, 0.5f, 0.2f);

			foreach (IMaterialLight light in lights)
			{
				if (!light.Enabled)
					continue;
				IMaterialDirectionalLight directional = light as IMaterialDirectionalLight;
				if (directional != null)
				{
					lightDirection += directional.Direction * Vector3.Dot(rgbScalar,directional.Colour);
				}
				else //must be a point light
				{
					IMaterialPointLight pointLight = light as IMaterialPointLight;
					if (pointLight != null)
					{
						...
						float attenuation = pointLight.ConstantAttenuation + distance * pointLight.LinearAttenuation + distanceSq * pointLight.QuadraticAttenuation;
						...
						lightDirection += direction * (Vector3.Dot(rgbScalar, pointLight.Colour) * weighting * attenuation);
					}
				}
			}
		}
		...
		private void AccumulateLightContribution(List<ShaderLight> lights, ref Vector3 centre, 
			ref Vector3 lightDirection, ref Vector3 ambientLight, ref Vector3 directionalLight, float directionalWeighting)
		{
			float invScaleSq = 0.5f / (scale * scale);
			...
			foreach (IMaterialLight light in lights)
			{
				...
				if (directional != null)
				{
					...
					directionalLight += directional.Colour * weight;
					ambientLight += directional.Colour * (1 - weight);
				}
				else //must be a point light
				{
					IMaterialPointLight pointLight = light as IMaterialPointLight;
					if (pointLight != null)
					{
						...
						Vector3 colour = pointLight.Colour;
		...
		private void CalculateBindPose()
		{
			...
			for (int i = 0; i < bindPoseWorld.Length; i++)
				bindPoseInverse[i] = new Transform(Matrix.Invert(pose[i]));

			this.sourceData.skeleton.TransformHierarchy(pose);

			for (int i = 0; i < bindPoseWorld.Length; i++)
			{
				bindPoseWorld[i] = new Transform(pose[i]);
				bindPoseWorldInverse[i] = new Transform(Matrix.Invert(pose[i]));
			}
	...
	public sealed class AvatarAnimationController : Processor.AnimationProcessor
	{
		...
		public AnimationInstance PlayPresetAnimation(Microsoft.Xna.Framework.GamerServices.AvatarAnimationPreset animation, bool useExpressions, float fadeInTime, float fadeOutTime)
		{
			...
			if (!ContentLoaded)
				throw new InvalidOperationException("AvatarInstance content has not been added");
			if (IsDisposed)
